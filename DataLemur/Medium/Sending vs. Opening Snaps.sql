...
-- Solution
with t as (
SELECT age_bucket,activity_type,sum(time_spent) ts 
FROM activities a
...
select age_bucket, 
       round(sum(case when activity_type='send' then ts end)*100.0/sum(ts),2) as send_perc,
       round(sum(case when activity_type='open' then ts end)*100.0/sum(ts),2) as open_perc
from t
group by age_bucket;
